...
const statusConfig = {
    'DEPLOYED': { color: '#4ade80', dot: true },
    'ACTIVE': { color: '#E8AA3A', dot: true },
    'LIVE': { color: '#4ade80', dot: true },
    'IN PROGRESS': { color: 'rgba(255,255,255,0.3)', dot: false },
};
...
export default function ProjectsGrid({ onSelect, selectedId }) {
    return (
        <section
            id="projects-grid"
        ...
        >
            ...
            <div className="absolute top-6 left-6 z-20 flex flex-col gap-1">
                <span className="font-display text-[9px] tracking-[0.35em] uppercase" style={{ color: 'var(--color-pipboy-green)' }}>■ PROJECT_ARCHIVE v3.0</span>
                <span className="font-display text-[8px] tracking-[0.2em] opacity-50" style={{ color: 'var(--color-pipboy-green)' }}>VAULT: FULL DOSSIER</span>
            </div>
            <div className="absolute top-6 right-6 z-20 text-right flex flex-col gap-1">
                <span className="font-display text-[9px] tracking-[0.2em] opacity-60" style={{ color: 'var(--color-vault-gold)' }}>{projects.length} OPS LOGGED</span>
                <span className="font-display text-[8px] tracking-[0.15em] opacity-35" style={{ color: 'var(--color-vault-gold)' }}>STATUS: ALL ACTIVE</span>
            </div>
            ...
            <div className="relative z-20 max-w-[1600px] mx-auto px-6 sm:px-10 lg:px-16">
                ...
                >
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <span className="font-display text-[10px] tracking-[0.5em] uppercase" style={{ color: 'var(--color-vault-gold)' }}>
                            // PROJECT_ARCHIVE: ALL_OPERATIONS
                        </span>
                        <motion.span
                    ...
                    <h2 className="font-display font-black uppercase leading-none">
                        <span className="block text-[clamp(2.5rem,6vw,5.5rem)] text-white"
                            style={{ textShadow: '0 0 40px rgba(46,110,101,0.45)' }}>
                            CASE
                        </span>
                        <span className="block text-[clamp(2.5rem,6vw,5.5rem)]"
                            style={{ WebkitTextStroke: '2px var(--color-vault-gold)', color: 'transparent', textShadow: '0 0 50px rgba(232,170,58,0.25)', filter: 'drop-shadow(0 0 12px rgba(232,170,58,0.2))' }}>
                            STUDIES
                        </span>
                    </h2>

                    <p className="font-sans text-sm mt-4 max-w-lg leading-relaxed" style={{ color: 'rgba(240,224,196,0.45)' }}>
                        Full dossier of classified engagements. Each entry documented by technology stack, impact assessment, and deployment status.
                    </p>
                    ...
                    {projects.map((project) => {
                        ...
                        return (
                            <motion.div
                                ...
                                style={{
                                    border: selectedId === project.id ? '1px solid rgba(232,170,58,0.6)' : '1px solid rgba(232,170,58,0.13)',
                                    background: selectedId === project.id ? 'rgba(232,170,58,0.04)' : 'rgba(7,13,12,0.75)',
                                    backdropFilter: 'blur(6px)',
                                    boxShadow: selectedId === project.id ? '0 0 30px rgba(232,170,58,0.1), inset 0 0 24px rgba(46,110,101,0.08)' : 'none',
                                }}
                                onClick={() => onSelect && onSelect(project)}
                                onMouseEnter={e => { if (selectedId !== project.id) { e.currentTarget.style.borderColor = 'rgba(232,170,58,0.5)'; e.currentTarget.style.boxShadow = '0 0 30px rgba(232,170,58,0.07), inset 0 0 24px rgba(46,110,101,0.06)'; } }}
                                onMouseLeave={e => { if (selectedId !== project.id) { e.currentTarget.style.borderColor = 'rgba(232,170,58,0.13)'; e.currentTarget.style.boxShadow = 'none'; } }}
                            >
                                {/* Top sweep */}
                                <div className="absolute top-0 left-0 w-0 h-[2px] transition-all duration-500 group-hover:w-full pointer-events-none"
                                    style={{ background: 'var(--color-vault-gold)' }} />
                                ...
                                {/* Corner brackets */}
                                <span className="absolute top-0 left-0 w-3 h-3 border-t border-l pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ borderColor: 'var(--color-vault-gold)' }} />
                                <span className="absolute top-0 right-0 w-3 h-3 border-t border-r pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ borderColor: 'var(--color-vault-gold)' }} />
                                <span className="absolute bottom-0 left-0 w-3 h-3 border-b border-l pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ borderColor: 'var(--color-vault-gold)' }} />
                                <span className="absolute bottom-0 right-0 w-3 h-3 border-b border-r pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ borderColor: 'var(--color-vault-gold)' }} />
                                ...
                                <div className="relative z-10 p-4 flex flex-col flex-1 gap-3">
                                    ...
                                    {/* Tag + title */}
                                    <div>
                                        <span className="inline-block font-display text-[9px] uppercase tracking-[0.2em] px-2 py-[2px] border mb-2"
                                            style={{ color: 'var(--color-vault-gold)', borderColor: 'rgba(232,170,58,0.3)', background: 'rgba(232,170,58,0.06)' }}>
                                            {project.tag}
                                        </span>
                                        <h3 className="font-display font-black text-white uppercase leading-tight transition-colors duration-300 group-hover:text-[--color-vault-gold]"
                                    ...
                                    {/* Description — hidden, shown in ProjectDetails */}

                                    {/* Access file button */}
                                    <div className="px-3 py-2 flex items-center gap-2" style={{ borderLeft: '2px solid rgba(46,110,101,0.5)', background: 'rgba(46,110,101,0.04)' }}>
                                        <motion.span
                                            animate={{ opacity: [1, 0, 1] }}
                                            transition={{ repeat: Infinity, duration: 1.1 }}
                                        ...
                                        <span className="font-display text-[8px] uppercase tracking-[0.3em]" style={{ color: 'rgba(46,110,101,0.7)' }}>
                                            {selectedId === project.id ? 'FILE OPEN' : 'CLICK TO ACCESS FILE'}
                                        </span>
                                    </div>

                                    {/* Impact */}
                                    <div className="px-3 py-2" style={{ borderLeft: '2px solid var(--color-vault-gold)', background: 'rgba(232,170,58,0.04)' }}>
                                        <span className="block font-display text-[7px] uppercase tracking-[0.35em] mb-[3px]" style={{ color: 'rgba(232,170,58,0.4)' }}>IMPACT</span>
                                        <span className="font-display text-[10px] font-bold text-white uppercase tracking-wide">{project.impact}</span>
                                    </div>

                                    {/* Stack pills */}
                                    <div className="flex flex-wrap gap-[5px]">
                                        {project.stack.map((tech) => (
                                            <span key={tech} className="font-display text-[8px] uppercase tracking-wider px-2 py-[2px]"
                                                style={{ color: 'rgba(240,224,196,0.3)', border: '1px solid rgba(255,255,255,0.08)' }}>
                                                {tech}
                                            </span>
                                        ))}
                                    </div>
                                    ...
                                    {/* Action links */}
                                    <div className="flex items-center gap-3 pt-3 border-t" style={{ borderColor: 'rgba(232,170,58,0.1)' }}>
                                        {project.github && (
                ...
                <div className="flex items-center justify-between mt-10 pt-5 border-t" style={{ borderColor: 'rgba(232,170,58,0.1)' }}>
